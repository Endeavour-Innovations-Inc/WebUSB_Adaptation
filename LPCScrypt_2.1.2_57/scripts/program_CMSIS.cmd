@echo off
REM Script to autodetect then program CMSIS-DAP image onto LPC-Link2 or LPCXpresso V2/V3 board
REM Arguments: Null, NB, Path to binary, Help
...
if /i "%Part%" == "Link2" (
    ...
    set Flash= SPIFI
    if /i "%1" == "NB" (
        set Link2ImageWild=LPC432x_CMSIS_DAP_NB_*.bin.hdr
    )
    if /i "%1" == "SER" (
        set Link2ImageWild=LPC432x_CMSIS_DAP_SER_*.bin.hdr
    ) 
    if /i "%1" == "" (
        set Link2ImageWild=LPC432x_CMSIS_DAP_*.bin.hdr
    )
    call :getLink2Image
)
...
if /i "%Part%" == "Lpcx" (
    ...
    set Flash= BANKA
    if /i "%1" == "NB" (
        set Link2ImageWild=LPC432x_IAP_CMSIS_DAP_NB_*.bin
    )
    if /i "%1" == "SER" (
        set Link2ImageWild=LPC432x_IAP_CMSIS_DAP_SER_*.bin
    )	
    if /i "%1" == "" (
...
:Usage
    ...
    echo                 [Programs Bridged CMSIS-DAP Image]
    echo NB              [Programs Non Bridged CMSIS-DAP Image]
    echo SER             [Programs VCOM SERial Bridge CMSIS-DAP Image]
...
:progsuccess
    echo.
    <NUL set /p =-%DeviceName% programmed successfully and has the unique ID: 
    lpcscrypt queryencode
